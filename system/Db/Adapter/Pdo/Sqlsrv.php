...
use Core\Db\Result\PdoSqlsrv as ResultPdo;
 ...
 * </code>.
 *
 * @property \Core\Db\Dialect\Sqlsrv $_dialect
 */
class Sqlsrv extends \Phalcon\Db\Adapter\Pdo\AbstractPdo implements \Phalcon\Db\Adapter\AdapterInterface
{
    ...
    public function connect(?array $descriptor = null): bool
    {
        ...
        // $dsn = "sqlsrv:Server=" . $descriptor['host'] . "," . $descriptor['port'] . ";Database=" . $descriptor['dbname'] . ";LoginTimeout=3";
        ...
        // Ambil & lepas 'options' dan 'attributes' (kalau ada)
        $options    = $descriptor['options']    ?? [];
        $attributes = $descriptor['attributes'] ?? [];
        unset($descriptor['options'], $descriptor['attributes']);

        // DSN params "ringan" (bisa dioverride via $options)
        $dsnParams = [
            'Encrypt'                  => $options['Encrypt']                  ?? 'Yes',
            'TrustServerCertificate'   => $options['TrustServerCertificate']   ?? 'Yes',
            'MultipleActiveResultSets' => $options['MultipleActiveResultSets'] ?? 'No',
            'LoginTimeout'             => $options['LoginTimeout']             ?? '3',
        ];
        ...
        // --- DEFAULT ATTR YG DIDUKUNG pdo_sqlsrv ---
        // Hindari ATTR_TIMEOUT & (opsional) ATTR_PERSISTENT
        $safeDefaults = [
            \PDO::ATTR_ERRMODE               => \PDO::ERRMODE_EXCEPTION,
            \PDO::ATTR_EMULATE_PREPARES      => false,
            \PDO::SQLSRV_ATTR_DIRECT_QUERY   => true,
            // \PDO::SQLSRV_ATTR_QUERY_TIMEOUT  => $options['QueryTimeout'] ?? 15, // detik per-query
            // \PDO::SQLSRV_ATTR_ENCODING       => \PDO::SQLSRV_ENCODING_UTF8,
            // \PDO::ATTR_PERSISTENT         => true, // ⚠️ KOMENTARI jika memicu unsupported
        ];

        // Gabung override user (kalau ada), tapi nanti tetap difilter
        $wanted = $safeDefaults + $attributes;

        // Buat PDO tanpa atribut dulu → set satu-per-satu agar yang tidak didukung di-skip
        try {
            $this->pdo = new \PDO($dsn, $dbusername, $dbpassword);
        } catch (\PDOException $e) {
            $safeDsn = preg_replace('/(PWD|Password|UID|User Id)=[^;]*/i', '$1=***', $dsn);
            throw new \PDOException("SQL Server connection failed. DSN={$safeDsn}; {$e->getMessage()}", (int)$e->getCode(), $e);
        }

        // Set atribut satu-per-satu; abaikan yang tidak didukung
        foreach ($wanted as $attr => $val) {
            try {
                $this->pdo->setAttribute($attr, $val);
            } catch (\Throwable $e) {
                // Unsupported attribute → skip
                // error_log("Skip unsupported PDO attribute {$attr}: " . $e->getMessage());
            }
        }

        // Dialect
        $dialectClass = $descriptor['dialectClass'] ?? ('Phalcon\\Db\\Dialect\\' . ucfirst($this->_dialectType));
    ...
    public function describeColumns(string $table, ?string $schema = null): array
    {
        ...
        foreach ($this->fetchAll($this->_dialect->describeColumns($table, $schema)) as $field) {
            ...
            */
            if ($field['NULLABLE'] == 0) {  // 0 = NOT NULL
                $definition['notNull'] = true;
            } else {  // 1 = NULLABLE, jangan set notNull
                $definition['notNull'] = false;  // Explicit set false untuk nullable
            }
