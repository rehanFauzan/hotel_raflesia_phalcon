window.defaultUrl = `${baseUrl}panel/master/master_bagian/`;
var table;
let rowData;
...
$(document).ready(function () {
	...
	formManage.validate({
		...
		errorPlacement: function(error, element) {
            error.addClass('fs-9'); // Add font-size 65% class
	...
	$('#btn-add').click(function (e) {
		...
		$('#direktorat_id').val(null).trigger('change');

		// Clear any hidden fields that might contain previous data
		$('#id_edit').val('');
		manageModal.find("#title_modal").text("Tambah Data Bagian")
	...
	$('#btn-edit').click(function (e) {
		...
		manageModal.find("#title_modal").text("Ubah Data Bagian")
		...
		if (!_.isEmpty(rowData.direktorat_id)) {
            formManage.find("#direktorat_id").select2("trigger", "select", {
                data: {
                    id: rowData.direktorat_id,
                    kode: rowData.direktorat_kode,
                    nama: rowData.direktorat_nama,
                    text: `(${rowData.direktorat_kode}) ${rowData.direktorat_nama}`,
                    selected: true
                }
            });
        }
...
function renderViewDatatableAction() {
	table = $("#datatables-bagian").DataTable({
		ajax: {
...
function renderSelect2Action() {
    $("#direktorat_id").select2({
		...
		ajax: {
			url: "{{ url('panel/referensi/getMasterDirektorat') }}",
	...
	$("#search_direktorat_id").select2({
		allowClear: true,
		theme: "bootstrap-5",
        selectionCssClass: "select2--small",
        dropdownCssClass: "select2--small",
		dropdownParent: $('#elFilterDirektorat'),
		placeholder: "Pilih Direktorat",
		ajax: {
			url: "{{ url('panel/referensi/getMasterDirektorat') }}",
			data: function (params) {
