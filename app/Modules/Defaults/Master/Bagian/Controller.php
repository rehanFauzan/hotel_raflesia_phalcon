...
use App\Modules\Defaults\Master\Bagian\BagianModel;
use App\Modules\Defaults\Master\Bagian\BagianViewModel;

/**
 * @routeGroup('/master/master_bagian')
...
class Controller extends BaseController
{
    ...
    public function datatableAction()
    {
        ...
        $search_direktorat_id = escape_xss($this->request->getPost('search_direktorat_id'));
        ...
        $builder = $this->modelsManager->createBuilder()
            ->columns('*')
            ->from(BagianViewModel::class)
            ->where("1=1")
            ->andWhere("pdam_id = :pdam_id:", ['pdam_id' => $pdam_id]);

        // Then use in your SQL WHERE clause
        if (!empty($search) && isset($search)) {
            $builder->andWhere("kode LIKE '%$search%' OR nama LIKE '%$search%' OR direktorat_nama LIKE '%$search%'");
        }
        ...
        if (!empty($search_direktorat_id) && isset($search_direktorat_id)) {
            $builder->andWhere("direktorat_id = '$search_direktorat_id'");
        }
    ...
    public function saveDataAction()
    {
        if ($this->request->isPost()) {
            ...
            $direktorat_id = escape_xss($this->request->getPost('direktorat_id'));
    ...
    public function updateDataAction()
    {
        if ($this->request->isPost()) {
            ...
            $existingUser = BagianModel::findFirst([
            ...
            if ($existingUser) {
                ...
                return Response::setJsonContent([
                    'error' => 1,
                    'message' => "Nama '$nama' sudah digunakan oleh Bagian lain. Silahkan gunakan Bagian yang berbeda."
                ]);
            }
    ...
    public function deleteDataAction()
    {
        if ($this->request->isPost()) {
            ...
            $findForDelete = BagianModel::findFirst([
