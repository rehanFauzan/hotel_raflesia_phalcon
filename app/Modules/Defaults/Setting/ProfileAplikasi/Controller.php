...
namespace App\Modules\Defaults\Setting\ProfileAplikasi;
...
/**
 * @routeGroup('/setting/profile-aplikasi')
...
class Controller extends BaseController
{
    ...
    {
        // Ambil dan bersihkan input dari formi
        $image_logo = escape_xss($this->request->getPost('image_logo'));
        $image_logo_temp = escape_xss($this->request->getPost('image_logo_temp'));
        ...
        // if $image_logo is empty, use the $image_logo_temp
        if (!isset($image_logo) && empty($image_logo)) {
            $image_logo = $image_logo_temp;
        }
        ...
        if ($setting->save()) {
            ...
            return $this->response->setJsonContent([
                'error' => 0,
                'message' => 'Data Profil Aplikasi berhasil diperbarui.',
                'redirect' => 'panel/setting/profile-aplikasi'
            ]);
        } else {
            return $this->response->setJsonContent([
                'error' => 1,
