...
return str.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
	...
	window.defaultUrl = `${baseUrl}panel/setting/role/`;
var title_modal = `Tambah Role`;
var table;
let rowData;
...
$(document).ready(function () {
	...
	$('#btn-add').click(function (e) {
		...
		$('#satuan_kerja_id').val(null).trigger('change');
		$('#state').val('').trigger('change');

		// Clear any hidden fields that might contain previous data
		$('#id_edit').val('');
		manageModal.find("#title_modal").text("Tambah Data Role")
	...
	$('#btn-edit').click(function (e) {
		...
		manageModal.find("#title_modal").text("Ubah Data Role")

		formManage.find('#input-action').val('update');
		formManage.find('#id_edit').val(rowData.id);
		formManage.find("#satuan_kerja_id").select2("trigger", "select", {
			data: {
		...
		formManage.find('#role').val(rowData.role);
		formManage.find('#status').val(rowData.state);
...
function renderViewDatatableAction() {
	table = $("#datatables-roles").DataTable({
		ajax: {
