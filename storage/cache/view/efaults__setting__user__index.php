...
return str.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
	...
	window.defaultUrl = `${baseUrl}panel/setting/user/`;
var title_modal = `Tambah User`;
var table;
let rowData;
...
$(document).ready(function () {
	...
	$('#btn-edit').click(function (e) {
		...
		manageModal.find("#title_modal").text("Ubah Data User")
		formManage.find('#input-action').val('update');
		formManage.find('#id_edit').val(rowData.id);


		formManage.find("#satker_id").select2("trigger", "select", {
			data: {
				id: rowData.kode_satker,
				nama: rowData.nama_satker,
				text: `(${rowData.kode_satker}) ${rowData.nama_satker}`,
				selected: true
			}
		});

		formManage.find('#nama').val(rowData.nama);
		formManage.find('#username').val(rowData.username);
		formManage.find('#username').addClass('disabled');

		// Decrypt password with CSRF token from PHP
		$.post("<?= $this->url->get('panel/referensi/decryptPassword') ?>", {
				password: rowData.pass_enc,
			},
			function (data, textStatus, jqXHR) {
				if (!_.isEmpty(data.data)) {
					formManage.find('#password').val(data.data);
					formManage.find('#password').removeClass('disabled');
				}
			},
			"JSON"
		);

		formManage.find("#role_id").select2("trigger", "select", {
			data: {
				id: rowData.id_role,
				nama: rowData.role,
				text: rowData.role,
		...
		formManage.find('#state').val(rowData.state);
	...
	$('#btn-submit').click(function (e) {
		...
		// let pgw_id = formManage.find('#pgw_id').val();
		// if (_.isEmpty(pgw_id)) {
		// 	notyf.error("Harap pilih pegawai!");
		// 	return false;
		// }
						...
						if (data == "1") {
							...
							return `
								<span class="badge badge-phoenix fs-10 badge-phoenix-danger">
									<span class="badge-label">Non Aktif</span>
...
function renderSelect2Action() {
	...
	$("#search_satker_id").select2({
		...
		placeholder: "Pilih Satuan Kerja",
		ajax: {
			url: "<?= $this->url->get('panel/referensi/getSatuankerja') ?>",
			data: function (params) {
				return {
					q: params.term,
					page: params.page || 1,
	...
	$("#search_role_id").select2({
		...
		placeholder: "Pilih Hak Akses",
		ajax: {
			url: "<?= $this->url->get('panel/referensi/getRolemaster') ?>",
			data: function (params) {
				return {
					q: params.term,
					page: params.page || 1,
